...
**작성 시작 전에 반드시 1~3편을 읽어라.** 또한 이 편은 기존 최적화 시리즈와 직접 연결되므로 `_posts/*-optimization-levenberg-marquardt.md`도 읽어라 — LM법 설명을 반복하지 말고 링크로 넘겨야 한다.

## 확정 사항

- **시리즈**: "XPS 기초" 전 5편 중 **4편**
- **slug**: `xps-quantification-peak-fitting`
- **포스트 경로**: `_drafts/xps-quantification-peak-fitting.md` — 날짜 없이 쓴다. 완성되면 `.claude/SCHEDULE.md`에서 발행일을 받아 `_posts/`로 옮긴다
...
- **1편의 약속을 여기서 지켜라.** 1편 5절은 "통과 에너지가 분해능과 감도를 맞바꾸는 원리는 4편에서 분석기 구조와 함께 다룬다"고 예고했다(서베이는 높은 통과 에너지, 고분해능 스캔은 낮은 통과 에너지). 이 절에서 한 단락으로 설명하라.
  - 반구형 분석기(concentric hemispherical analyzer, CHA)는 들어온 전자를 먼저 감속해 고정된 통과 에너지 $E_{pass}$로 맞춘 뒤 두 반구 사이를 지나게 한다(FAT, fixed analyzer transmission 모드).
  - 분석기의 절대 분해능은 $E_{pass}$에 비례한다(비례 상수는 슬릿 폭과 반구 반지름이 정한다). 그래서 $E_{pass}$를 낮추면 분해능이 좋아지고, 대신 통과하는 전자 수가 줄어 신호가 약해진다. FAT 모드에서는 이 분해능이 스펙트럼 전체에서 일정하다.
  - 감속비 $E_K/E_{pass}$가 투과함수 $T$에 들어가므로, 같은 장비라도 $E_{pass}$를 바꾸면 RSF가 달라진다 → 바로 위 항목의 근거.
  - 분석기 분해능은 5절 Gaussian 폭의 장비 기여로 이어진다.
...
- 반구형 분석기의 분해능 식을 수식으로 쓰려면(예: $\Delta E/E_{pass}$를 슬릿 폭·반지름·입사각 반각으로 적는 식) 교재나 논문에서 형태를 확인하고 출처를 참고자료에 넣어라. 장비 수치(슬릿 폭, 반지름, 통과 에너지 값)를 지어내지 마라.
...
- 작성 후 `grep -n '\$[^$]*|' _drafts/xps-quantification-peak-fitting.md`로 수식 내 리터럴 `|` 확인.
