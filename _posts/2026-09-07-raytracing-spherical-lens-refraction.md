...
학교나 연구실에서 광선추적(ray tracing) 혹은 광학 설계 도구를 지원하지 않는 경우가 종종 있다. 소속 연구실도 그러했고, 필요성이 그리 크지는 않았다. 설계하는 광학계가 무한보정(infinity-corrected) 대물렌즈를 쓴 데다, 사용하는 가시광 영역에서 성능이 보장된 기성 광부품을 조합했기 때문이다. 무한보정계는 대물렌즈와 튜브렌즈 사이 구간이 평행광이어서 그 구간의 길이가 결상에 영향을 주지 않으므로, 필터나 빔스플리터를 넣고 빼도 초점이 흔들리지 않는다. 그러나 점점 더 정밀한 측정 데이터가 필요해지고 고배율 렌즈의 후초점면(back focal plane, BFP)을 관찰할 일이 많아지면서, 지그를 제작하고 초점거리를 특정할 광선추적 도구를 연구실에서 직접 구현해 사용하게 되었다.

기본 골자는 MATLAB 오픈소스 코드였고, 이를 Python으로 옮기고 다듬는 작업을 맡았다. 함수가 60개를 넘고 렌즈·미러·프리즘·빔스플리터·조리개까지 부품 종류도 많았던 탓에, 이식을 마친 뒤에 남은 문제는 옮긴 코드가 제대로 동작하는지를 어떻게 확인하는가였다. 겉보기에 그럴듯한 광선 그림이 나와도, 굴절 각도가 미묘하게 틀렸거나 렌즈 가장자리 근처에서 엉뚱한 점을 골랐다면 눈으로는 판별할 수 없다. 이 시리즈에서는 기하광학에 관한 간략한 소개와 함께, 부품별 구현 원리와 그 검증 방법을 하나씩 정리한다.

첫 편의 대상은 가장 기초가 되는 계산, 즉 광선이 구면(spherical surface)을 지나며 굴절되는 과정이다. 원리부터 다시 짚어가며 구현하고 그 결과를 검증한 과정을 정리한다. 이 글의 코드는 검증에 필요한 부분(`make_spherical_lens`, `refract_through_lens` 등)만 뽑아 재구성한 것이다(전체 코드: `_code/raytracing-spherical-lens-refraction/`).

## 1. 광선과 구면의 교차 — 두 개의 후보해

렌즈 하나를 통과하는 광선을 계산하려면 제일 먼저 "광선이 렌즈의 어느 면을 어디서 만나는가"부터 알아야 한다. 코드에서 광선은 `[x, y, vx, vy]` 네 값으로 표현된다. $(x_0, y_0)=(x, y)$는 지금 계산하는 면을 향해 출발하는 위치이고, $(v_x, v_y)$는 그 방향벡터다. 렌즈의 첫 면(광원에서 먼저 만나는 면)을 계산할 때는 $(x_0, y_0)$가 광원의 위치이지만, 렌즈를 통과해 두 번째 면을 계산할 때는 첫 면에서 방금 굴절되어 나온 점이 새로운 $(x_0, y_0)$가 된다. 즉 광원 자체가 아니라 이 면을 향해 지금 광선이 출발하는 자리다. 이 시작점과 방향벡터를 직선의 일반형으로 쓰면 이후 계산이 간단해진다.
...
구면은 평면이 아니므로, 렌즈 정점(vertex)에서의 접평면을 기준으로 재면 광축에서 멀어질수록 그 평면으로부터 점점 더 파고 들어간다. 이 파고 들어간 깊이를 sag(처짐량)라고 부르고, 렌즈 가장자리($D/2$)에서는 $\mathrm{sag} = R - \sqrt{R^2 - (D/2)^2}$로 계산된다. `offset`은 이 sag에 작은 배율(0.01)을 곱한 여유값이다. 부동소수점 계산 오차 때문에 렌즈 경계에 딱 걸쳐 있는 점이 "경계 밖"으로 잘못 판정되는 것을 막아준다.
...
이 렌즈에 평행광을 통과시키면 다음과 같은 경로가 나온다.
...
렌즈는 원점에 놓았고, 광원은 $x=1000$mm에서 $-x$ 방향으로 진행하는 평행광 11가닥(지름 200mm)이다. 여기서 한 가지 주의할 점이 있다. 광원이 $+x$에서 $-x$로 진행하므로 R1 면(첫 번째로 정의한 면)을 먼저 만날 것 같지만, `find_entry_surface`가 반환하는 진입면은 2번(R2)이다. R1과 R2는 렌즈를 정의할 때의 매개변수 순서일 뿐, 광원에서 봤을 때의 물리적 좌우와 대응하지 않는다. 이 렌즈는 R1=1000(볼록), R2=-1000(볼록)이고, 두 면의 정점(vertex) 좌표를 직접 계산하면 R1 면이 $x=-t/2$, R2 면이 $x=+t/2$에 있다. 즉 $+x$ 쪽에서 오는 광선은 R2 면을 먼저 만난다. 이 부호 규약은 2절의 가역성 검증에서 다시 나온다.

## 2. 검증 — 가역성과 이론값 대조

### 광학 가역성

광학 가역성(optical reversibility)이란 광선이 지나온 경로를 반대 방향으로 그대로 되짚어 갈 수 있다는 원리다. Snell's law가 시간 역전에 대해 대칭이기 때문에 성립한다 — 굴절된 광선의 진행 방향을 반대로 뒤집어 쏘면, 원래 들어왔던 경로를 정확히 되짚어 나가야 한다. 이는 `refract_through_lens`의 두 분기(`entry_surface==1`일 때와 그 반대일 때)가 서로 모순 없이 같은 물리를 구현했는지 보는 교차검증이 된다. 정방향 진입은 한쪽 분기를, 역추적은 반대쪽 분기를 타기 때문이다.
...
가역성 검증은 `refract_through_lens`의 두 분기가 서로 일관되게 동작하는지는 보여주지만, 그 결과가 실제 렌즈의 물리량과 맞는지는 확인해주지 않는다 — 두 분기가 똑같이 틀렸다면 가역성은 그래도 성립하기 때문이다. 그래서 이 광선추적 코드와 완전히 무관하게 유도된 독립적인 이론식과 대조해본다. 렌즈의 두께까지 고려한 thick-lens 공식으로 초점거리와 후방초점거리(back focal distance, BFD)를 구할 수 있다.

$$ \frac{1}{f} = (n-1)\left[\frac{1}{R_1} - \frac{1}{R_2} + \frac{(n-1)t}{n R_1 R_2}\right], \qquad \mathrm{BFD} = f\left[1 - \frac{(n-1)t}{n R_1}\right] $$

BFD 식의 $R_1$은 광선이 먼저 만나는 면의 곡률반경이다. 1절에서 확인했듯 이 배치에서 광선이 먼저 만나는 면은 R2이지만, $\lvert R_1 \rvert = \lvert R_2 \rvert$인 대칭 렌즈이므로 어느 쪽으로 들어가든 BFD 값은 같다.

이 공식은 근축광선(paraxial ray, 광축에 아주 가까운 광선) 근사 위에서 유도된 것이다. 광선추적 시뮬레이션은 근사 없이 실제 기하로 계산하므로, 광축에서 조금이라도 떨어진(marginal) 광선이 이 공식과 정확히 일치할 이유가 없다. 오히려 정확히 일치하는 쪽이 이상하다. 이 차이가 구면수차(spherical aberration)다.
...
다만 이번에 검증한 `refract_through_lens`는 전반사(total internal reflection) 여부를 판정하지 않는다. Snell's law에서 $\sin\theta_2 = (n_1/n_2)\sin\theta_1$이 1을 넘어서면 `arcsin`이 정의역을 벗어나는데, 이 함수는 그 경우를 걸러내지 않고 NumPy가 반환한 `NaN`을 다음 계산으로 그대로 흘려보낸다. MATLAB 원본도 이 구면렌즈 함수에서는 마찬가지였으며, 전반사를 명시적으로 판정하는 것은 프리즘 계산 함수 쪽뿐이었다. 렌즈는 대개 공기보다 굴절률이 높은 매질로 들어가는 각이 완만해 전반사 조건에 걸리는 일이 드물지만, 곡률이 크고 조리개가 넓은 조합에서는 걸릴 수 있다. 다음 편에서는 프리즘의 전반사 판정 로직과, 그것을 프리즘이 아닌 렌즈에 적용하면 어떻게 되는지를 다룬다.
