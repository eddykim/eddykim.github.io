"""엘립소메트리 배경이론 3편 그림 5개 생성 (한국어판·영문판).

실행: python generate_figures.py
출력: ../../assets/img/posts/ellipsometry-thin-film-multilayer-reflectance/      (한국어)
      ../../assets/img/posts/ellipsometry-thin-film-multilayer-reflectance/en/   (영문)

계산에 무작위 요소가 없으므로 두 언어의 그림은 데이터가 완전히 동일하고
표기만 다르다.
"""
import os
...
LABELS = {
    "ko": {
        ...
        "rouard_sub": "하부층 → 상부층\n등가계면으로 순차 치환",
        "tmm": "전달행렬법 (TMM)",
        "tmm_sub": "행렬을 한 줄로 순서대로 곱함\n(위상+계면이 한 행렬 $Q_m$에 결합)",
        "smm": "산란행렬법 (SMM)",
        "smm_sub": "계면(빨강)과 층(초록)이\n독립된 행렬로 분리됨",
        "wavelength": "파장 (nm)", "reflectance": "반사율 R", "angle": "입사각 (deg)",
        "fig3": "SiO$_2$/Si 박막의 TMM 반사율 스펙트럼 (수직입사)",
        "fig4": "SiO$_2$({:.0f}nm)/Si — 입사각×파장에 대한 " + r"$\Psi, \Delta$" + " (SMM)",
        "model_a": "모델 A: d={:.0f}nm, n={:.3f}",
        "model_b": "모델 B: d={:.0f}nm, n={:.3f}",
        "max_gap": "최대 차이 {:.2f}°",
        "max_gap_at": "최대 차이 {:.2f}° @ {:.1f}°",
        "panel_a": "(a) 단일 입사각 {:.0f}° · 파장 스캔",
        "panel_b": "(b) 단일 파장 {:.0f}nm · 입사각 스캔",
        "fig5": "좁은 파장 대역에서 서로 구별되지 않는 두 (d, n) 조합",
    },
    "en": {
        ...
        "tmm_sub": "matrices multiplied in one chain\n(phase and interface combined in $Q_m$)",
        "smm": "Scattering Matrix Method",
        "smm_sub": "interfaces (red) and layers (green)\nkept as separate matrices",
        "wavelength": "Wavelength (nm)", "reflectance": "Reflectance R",
        "angle": "Angle of incidence (deg)",
        "fig3": "TMM reflectance spectra of SiO$_2$/Si films (normal incidence)",
        "fig4": "SiO$_2$({:.0f}nm)/Si — " + r"$\Psi, \Delta$" + " over angle x wavelength (SMM)",
        "model_a": "Model A: d={:.0f}nm, n={:.3f}",
        "model_b": "Model B: d={:.0f}nm, n={:.3f}",
        "max_gap": "max difference {:.2f}°",
        "max_gap_at": "max difference {:.2f}° @ {:.1f}°",
        "panel_a": "(a) single angle {:.0f}° - wavelength scan",
        "panel_b": "(b) single wavelength {:.0f}nm - angle scan",
        "fig5": "Two (d, n) pairs indistinguishable over a narrow band",
    },
}
...
# ---------------------------------------------------------------------------
# 계산에 쓰는 공통 상수와 헬퍼 (그림과 진단 출력이 함께 쓴다)
# ---------------------------------------------------------------------------
...
def render(L):
    ...
    ax.text(4.6 * dx, 1.15, r"Air : $N_0$", fontsize=11, **L["font"])
    ax.text(4.6 * dx, -d / 2, r"Layer : $N_1$", fontsize=11, va="center", **L["font"])
    ax.text(4.6 * dx, -d - 0.55, r"Substrate : $N_2$", fontsize=11, **L["font"])
    ...
    ax.text(*(p_inc_start + [0.06, 0.03]), r"$E_i$", color="tab:blue", fontsize=12, **L["font"])

    er1_end = O + L_in * np.array([np.sin(theta0), np.cos(theta0)])
    ray(O, er1_end, "tab:red")
    ax.text(*(er1_end + [0.03, 0.02]), r"$E_{r1}$", color="tab:red", fontsize=11, **L["font"])
    ...
    ax.text((O[0] + A[0] + B[0]) / 3, -d * 0.62,
            ...
            fontsize=8.5, ha="center", **L["font"])
    ax.annotate(L["phase_diff"], xy=(B[0] + 0.15, 0.42),
                fontsize=10.5, color="black", **L["font"])
    ...
    ax.set_aspect("equal")
    ax.axis("off")
    ax.set_title(L["fig1"], fontsize=12, **L["font"])

    fig.tight_layout()
    fig.savefig(os.path.join(L["dir"], "fig1-concept-diagram.png"), dpi=150)
    plt.close(fig)
    ...
    def box(ax, xy, w, h, text, fc, ec="black", fontsize=10):
        ...
        ax.text(xy[0] + w / 2, xy[1] + h / 2, text, ha="center", va="center",
                fontsize=fontsize, zorder=4, **L["font"])
    ...
    ax.text(0.5, 1.16, L["rouard"], ha="center", fontsize=13, weight="bold", **L["font"])
    ax.text(0.5, -0.12, L["rouard_sub"], ha="center",
            fontsize=9, color="#555", **L["font"])
    ...
    ax.text(0.5, 1.16, L["tmm"], ha="center", fontsize=13, weight="bold", **L["font"])
    ax.text(0.5, 0.20, L["tmm_sub"],
            ha="center", fontsize=9, color="#555", **L["font"])
    ...
    ax.text(0.5, 1.16, L["smm"], ha="center", fontsize=13, weight="bold", **L["font"])
    ax.text(0.5, 0.20, L["smm_sub"],
            ha="center", fontsize=9, color="#555", **L["font"])
    ...
    fig2.tight_layout()
    fig2.savefig(os.path.join(L["dir"], "fig2-methods-comparison.png"), dpi=150)
    plt.close(fig2)
    ...
    ax3.set_xlabel(L["wavelength"], fontsize=11, **L["font"])
    ax3.set_ylabel(L["reflectance"], fontsize=11, **L["font"])
    ax3.set_title(L["fig3"], fontsize=12, **L["font"])
    ax3.set_xlim(400, 1000)
    ax3.set_ylim(0, 1)
    ax3.legend(prop=L["legend"], fontsize=10)
    ax3.grid(alpha=0.3)

    fig3.tight_layout()
    fig3.savefig(os.path.join(L["dir"], "fig3-thickness-fringes.png"), dpi=150)
    plt.close(fig3)
    ...
    ax4a.set_title(r"$\Psi$ (deg)", fontsize=12, **L["font"])
    fig4.colorbar(im_a, ax=ax4a)

    im_b = ax4b.pcolormesh(wavelengths_2d, angles_deg, delta_deg, cmap="twilight", shading="auto")
    ax4b.set_title(r"$\Delta$ (deg)", fontsize=12, **L["font"])
    fig4.colorbar(im_b, ax=ax4b)

    for ax in (ax4a, ax4b):
        ax.set_xlabel(L["wavelength"], fontsize=11, **L["font"])
    ax4a.set_ylabel(L["angle"], fontsize=11, **L["font"])
    fig4.suptitle(L["fig4"].format(d_fixed),
                  fontsize=12, **L["font"])

    fig4.tight_layout()
    fig4.savefig(os.path.join(L["dir"], "fig4-angle-wavelength-map.png"), dpi=150)
    plt.close(fig4)
    ...
    labA = L["model_a"].format(MODEL_A[0], MODEL_A[1])
    labB = L["model_b"].format(MODEL_B[0], MODEL_B[1])
    ...
    for ax, (yA, yB), name in [(axes5[0, 0], (pA_wl.ravel(), pB_wl.ravel()), r"$\Psi$"),
        ...
        ax.text(0.03, 0.10, L["max_gap"].format(gap), transform=ax.transAxes,
                fontsize=10, color="#333", **L["font"])
    axes5[1, 0].set_xlabel(L["wavelength"], fontsize=11, **L["font"])
    axes5[0, 0].set_title(L["panel_a"].format(ang_fixed), fontsize=12, **L["font"])
    ...
    for ax, (yA, yB), name in [(axes5[0, 1], (pA_ang.ravel(), pB_ang.ravel()), r"$\Psi$"),
        ...
        ax.text(0.03, 0.10, L["max_gap_at"].format(diff[i], ang_scan[i]),
                transform=ax.transAxes, fontsize=10, color="#333", **L["font"])
    axes5[1, 1].set_xlabel(L["angle"], fontsize=11, **L["font"])
    axes5[0, 1].set_title(L["panel_b"].format(wl_fixed), fontsize=12, **L["font"])

    for ax in axes5.ravel():
        ax.grid(alpha=0.3)
    axes5[0, 0].legend(prop=L["legend"], fontsize=9, loc="upper right")

    fig5.suptitle(L["fig5"], fontsize=12, **L["font"])
    fig5.tight_layout()
    fig5.savefig(os.path.join(L["dir"], "fig5-single-vs-angle-resolved.png"), dpi=150)
    plt.close(fig5)



for _lang, _labels in LABELS.items():
    os.makedirs(_labels["dir"], exist_ok=True)
    print(f"--- [{_lang}] {os.path.normpath(_labels['dir'])} ---")
    render(_labels)
...
# ---------------------------------------------------------------------------
_L = LABELS["ko"]
_pA_wl, _dA_wl = psi_delta(*MODEL_A, ang_fixed, wl_narrow)
_pB_wl, _dB_wl = psi_delta(*MODEL_B, ang_fixed, wl_narrow)
_pA_ang, _dA_ang = psi_delta(*MODEL_A, ang_scan, wl_fixed)
_pB_ang, _dB_ang = psi_delta(*MODEL_B, ang_scan, wl_fixed)
_dd = np.abs(_dA_ang - _dB_ang).ravel()
_i = _dd.argmax()

print()
print("=== 그림5: 두 모델의 구별 가능성 ===")
print(f"  모델 A: d={MODEL_A[0]:.2f}nm, n={MODEL_A[1]:.3f}")
print(f"  모델 B: d={MODEL_B[0]:.2f}nm, n={MODEL_B[1]:.3f}")
print(f"  (a) {ang_fixed:.0f}deg, {wl_narrow[0]:.0f}-{wl_narrow[-1]:.0f}nm 좁은 대역:")
print(f"      dPsi_max={np.abs(_pA_wl - _pB_wl).max():.4f}deg, "
      f"dDelta_max={np.abs(_dA_wl - _dB_wl).max():.4f}deg")
print(f"  (b) {wl_fixed:.0f}nm, 입사각 {ang_scan[0]:.0f}-{ang_scan[-1]:.0f}deg 스캔:")
print(f"      dDelta_max={_dd[_i]:.2f}deg @ {ang_scan[_i]:.1f}deg")
print(f"      Delta(A) 가 70-80deg 구간에서 지나는 값: "
      f"{np.round(_dA_ang.ravel()[(ang_scan >= 70) & (ang_scan <= 80)][::12], 1)}")

# 같은 두 모델을 넓은 대역에서 비교하면 단일 입사각만으로도 이미 갈린다.
_wl_wide = np.linspace(400, 1000, 400)
_pAw, _dAw = psi_delta(*MODEL_A, ang_fixed, _wl_wide)
_pBw, _dBw = psi_delta(*MODEL_B, ang_fixed, _wl_wide)
_wide_gap = max(np.abs(_pAw - _pBw).max(), np.abs(_dAw - _dBw).max())
print(f"  (c) {ang_fixed:.0f}deg, {_wl_wide[0]:.0f}-{_wl_wide[-1]:.0f}nm 넓은 대역:")
print(f"      최대 차이={_wide_gap:.2f}deg (좁은 대역의 "
      f"{_wide_gap / max(np.abs(_pA_wl - _pB_wl).max(), np.abs(_dA_wl - _dB_wl).max()):.0f}배)")
